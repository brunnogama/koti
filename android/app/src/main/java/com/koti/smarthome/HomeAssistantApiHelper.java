...
public class HomeAssistantApiHelper {
    ...
    public static void toggleEntity(Context context, String entityId) {
        new Thread(() -> {
            try {
                ...
                String host = "10.0.0.32";
                String token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJkOTNkYzMzNThhNjM0Yjk5YTI2NTBmNmQ3YzBjZmE2YSIsImlhdCI6MTc4ODY2NDE3MSwiZXhwIjoyMTA0MDI0MTcxfQ.Lw1amud6b6Fs3moZXlfIYzVe21aT4DFeve-fanx6cXA";
    ...
    public static void callService(Context context, String domain, String service, String entityId) {
        new Thread(() -> {
            try {
                SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
                String configJson = prefs.getString("koti_ha_connection_config", null);
                ...
                String jsonPayload = entityId.equals("all") ? "{}" : "{\"entity_id\":\"" + entityId + "\"}";
                try (OutputStream os = conn.getOutputStream()) {
                    os.write(jsonPayload.getBytes("UTF-8"));
                }

                int code = conn.getResponseCode();
                Log.d(TAG, "HA CallService Response: " + code);
                conn.disconnect();
            } catch (Exception e) {
                Log.e(TAG, "Error calling service " + domain + "." + service, e);
            }
        }).start();
    }
}
